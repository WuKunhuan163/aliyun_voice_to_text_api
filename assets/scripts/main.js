...
class VoiceRecognitionTester {
    ...
    getBrowserPermissionGuide() {
        ...
        if (isChrome) {
            guide += 'Chrome浏览器设置：点击地址栏左侧的图标（可能是锁、钥匙或信息图标），找到麦克风选项并选择允许，然后刷新页面重新尝试。或者点击右上角菜单进入设置，选择隐私和安全-网站设置-麦克风，确保麦克风设置为允许并在允许列表中添加本网站。';
        } else if (isFirefox) {
            guide += 'Firefox浏览器设置：点击地址栏左侧的图标（锁、盾牌或信息图标），点击权限选项卡，找到使用麦克风并选择允许，然后刷新页面重新尝试。或者通过菜单进入设置-隐私与安全-权限-麦克风设置，找到本网站并将状态改为允许。';
        } else if (isSafari) {
            guide += 'Safari浏览器设置：点击地址栏左侧的图标（可能显示为AA、锁或其他图标），将麦克风设置为允许，然后刷新页面重新尝试。或者通过Safari菜单进入偏好设置-网站-麦克风，找到本网站并选择允许。';
        } else if (isEdge) {
            guide += 'Edge浏览器设置：点击地址栏左侧的图标（锁、钥匙或信息图标），点击权限并找到麦克风选项，选择允许，然后刷新页面重新尝试。';
        } else {
