...
class VoiceRecognitionTester {
    ...
    getBrowserPermissionGuide() {
        ...
        if (isChrome) {
            ...
            guide += 'Firefox浏览器设置：点击地址栏左侧的锁图标，点击权限选项卡，找到使用麦克风并选择允许，然后刷新页面重新尝试。或者通过菜单进入设置-隐私与安全-权限-麦克风设置，找到本网站并将状态改为允许。';
        } else if (isSafari) {
            guide += 'Safari浏览器设置：点击地址栏左侧的网站设置图标，将麦克风设置为允许，然后刷新页面重新尝试。或者通过Safari菜单进入偏好设置-网站-麦克风，找到本网站并选择允许。';
        } else if (isEdge) {
            guide += 'Edge浏览器设置：点击地址栏左侧的锁图标，点击权限并找到麦克风选项，选择允许，然后刷新页面重新尝试。';
        } else {
            guide += '浏览器设置：点击地址栏左侧的锁图标或权限图标，找到麦克风权限并设置为允许，然后刷新页面重新尝试。';
        }
        
        guide += ' 通用解决方案：确保网站使用HTTPS访问，重新加载页面后再次尝试，清除浏览器缓存和Cookie，检查系统麦克风设备是否正常工作，或重启浏览器后重新尝试。';
        
        if (isMobile) {
            guide += ' 移动端注意：确保使用HTTPS访问网站，检查系统设置中的浏览器麦克风权限，某些浏览器需要在系统设置中单独允许。';
        }
